    ...
    public class ReferenciasController : Controller
    {
        ...
        public ActionResult Index(string buscar = "", int? codDesde = null, int? codHasta = null, string tipoDesde = "", string tipoHasta = "", DateTime? fechaAltaDesde = null, DateTime? fechaAltaHasta = null, DateTime? fechaBajaDesde = null, DateTime? fechaBajaHasta = null)
        {
            ...
            using (var con = new MySqlConnection(GetConnectionString()))
            {
                ...
                {
                    sql += "AND (codigo LIKE @buscar OR articulo LIKE @buscar OR descripcion LIKE @buscar) ";
                }
                if (codDesde.HasValue)
                {
                    sql += "AND codigo >= @codDesde ";
                }
                if (codHasta.HasValue)
                {
                    sql += "AND codigo <= @codHasta ";
                }
                if (!string.IsNullOrEmpty(tipoDesde))
                {
                    sql += "AND tipo >= @tipoDesde ";
                }
                if (!string.IsNullOrEmpty(tipoHasta))
                {
                    sql += "AND tipo <= @tipoHasta ";
                }
                if (fechaAltaDesde.HasValue)
                {
                    sql += "AND fecha_alta >= @fechaAltaDesde ";
                }
                if (fechaAltaHasta.HasValue)
                {
                    sql += "AND fecha_alta <= @fechaAltaHasta ";
                }
                if (fechaBajaDesde.HasValue)
                {
                    sql += "AND fecha_baja >= @fechaBajaDesde ";
                }
                if (fechaBajaHasta.HasValue)
                {
                    sql += "AND fecha_baja <= @fechaBajaHasta ";
                }
        ...
        public ActionResult Guardar(ArticuloModel a)
        {
            ...
            // 1. Forzar la lectura manual de los decimales cambiando cualquier coma por punto
            if (!string.IsNullOrEmpty(Request.Form["longitud"]))
            {
                a.longitud = Convert.ToDecimal(Request.Form["longitud"].Replace(",", "."), culture);
                ModelState.Remove("longitud");
            }
            if (!string.IsNullOrEmpty(Request.Form["altura"]))
            {
                a.altura = Convert.ToDecimal(Request.Form["altura"].Replace(",", "."), culture);
                ModelState.Remove("altura");
            }
            if (!string.IsNullOrEmpty(Request.Form["anchura"]))
            {
                a.anchura = Convert.ToDecimal(Request.Form["anchura"].Replace(",", "."), culture);
                ModelState.Remove("anchura");
            }
            if (!string.IsNullOrEmpty(Request.Form["iva"]))
            {
                a.iva = Convert.ToDecimal(Request.Form["iva"].Replace(",", "."), culture);
                ModelState.Remove("iva");
            }
            if (!string.IsNullOrEmpty(Request.Form["dto_base"]))
            {
                a.dto_base = Convert.ToDecimal(Request.Form["dto_base"].Replace(",", "."), culture);
                ModelState.Remove("dto_base");
            }
            if (!string.IsNullOrEmpty(Request.Form["huella_carbono"]))
            {
                a.huella_carbono = Convert.ToDecimal(Request.Form["huella_carbono"].Replace(",", "."), culture);
                ModelState.Remove("huella_carbono");
            }

            // 2. Forzar lectura manual de fechas en formato estándar ISO (yyyy-MM-dd)
            ...
            if (!string.IsNullOrEmpty(Request.Form["fecha_baja"]))
            {
                ...
                {
                    a.fecha_baja = fBaja;
                    ModelState.Remove("fecha_baja");
                }
            }

            if (!ModelState.IsValid)
            {
                CargarDesplegables(a);
                return View("AddOrEdit", a);
            }
            ...
            using (var con = new MySqlConnection(GetConnectionString()))
            {
                ...
                if (!existe)
                {
                    if (a.codigo != 0)
                    {
                        sql = "INSERT INTO articulo (codigo, articulo, descripcion, longitud, altura, anchura, tipo, unidad_medida, pais_origen, iva, es_reutilizable, huella_carbono, dto_base, fecha_alta, fecha_baja, activo, observaciones) " +
                              "VALUES (@codigo, @articulo, @descripcion, @longitud, @altura, @anchura, @tipo, @unidad_medida, @pais_origen, @iva, @es_reutilizable, @huella_carbono, @dto_base, @fecha_alta, @fecha_baja, @activo, @observaciones)";
                    }
                    else
                    {
