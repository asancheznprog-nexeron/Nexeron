...
{
    public class ComprasAlbaranesController : Controller
    {
        public ActionResult Index()
        {
            ...
            List<compras_albaranes> lista = new List<compras_albaranes>();
            ...
            using (MySqlConnection conexion = new MySqlConnection(connStr))
            {
                try
                {
                    ...
                    using (var cmd = conexion.CreateCommand())
                    {
                        cmd.CommandText = @"
                            ...
                            FROM (
                                SELECT a.NUMALB, a.FECHALB, a.NUMPEDIDO, a.CUENTA, c.NOMBRE_FISCAL as NombreProveedor,
                                ...
                                LEFT JOIN proveedores c ON a.CUENTA = c.CUENTA COLLATE utf8mb4_spanish_ci
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                lista.Add(new compras_albaranes
                                {
                ...
                {
                    ViewBag.Error = "Error al compilar el panel de albaranes de compra: " + ex.Message;
                }
            }
            return View(lista);
        }
                        ...
                        cmd.CommandText = @"
                            SELECT a.*, 
                                   (SELECT IF(COUNT(DISTINCT sub.ESTADOLIN) > 1, '104', a.ESTADO) 
                                    FROM compras_albaranes sub WHERE sub.NUMALB = a.NUMALB) as ESTADO_CALCULADO,
            ...
            using (MySqlConnection conexion = new MySqlConnection(connStr))
            {
                ...
                using (var cmd = conexion.CreateCommand())
                {
                    cmd.CommandText = @"
                        ...
                        LEFT JOIN proveedores c ON p.CUENTA = c.CUENTA COLLATE utf8mb4_spanish_ci
                        WHERE p.CUENTA = @cuenta AND EXISTS (
                            SELECT 1 FROM compras_pedidos sub 
                ...
                using (var cmd = conexion.CreateCommand())
                {
                    cmd.CommandText = @"
                        SELECT p.NUMLINEA, p.ARTI, p.DESARTI, p.CANTI as CantidadOriginal, 
                               ...
                               IFNULL((SELECT SUM(a.CANTI) FROM compras_albaranes a WHERE a.NUMPEDIDO = p.NUMPEDIDO AND a.NUMLINEAPED = p.NUMLINEA), 0) as CantidadAlbaranada,
                               (p.CANTI - IFNULL((SELECT SUM(a.CANTI) FROM compras_albaranes a WHERE a.NUMPEDIDO = p.NUMPEDIDO AND a.NUMLINEAPED = p.NUMLINEA), 0)) as Pendiente
                        FROM compras_pedidos p
        ...
        public ActionResult Crear(FormCollection form, string lineasJson)
        {
            ...
            List<compras_albaranes> lineasArticulos = new List<compras_albaranes>();
            try
            {
                var serializer = new JavaScriptSerializer();
                lineasArticulos = serializer.Deserialize<List<compras_albaranes>>(lineasJson);
            }
            catch (Exception ex)
            {
            ...
            using (MySqlConnection conexion = new MySqlConnection(connStr))
            {
                ...
                using (MySqlTransaction transaccion = conexion.BeginTransaction())
                {
                    try
                    {
                        ...
                        foreach (var linea in lineasArticulos)
                        {
                            using (var cmd = conexion.CreateCommand())
                            {
                                ...
                                cmd.CommandText = @"INSERT INTO compras_albaranes (
                    ...
                    catch (Exception ex)
                    {
                        ...
                        TempData["Error"] = "Error al crear el albarán de compra: " + ex.Message;
                        TempData["TipoError"] = "Crear";
                    }
                }
            }
        ...
        public ActionResult Actualizar(FormCollection form, string lineasJsonModificadas)
        {
            ...
            List<compras_albaranes> nuevasLineas = new List<compras_albaranes>();
            try
            {
                var serializer = new JavaScriptSerializer();
                nuevasLineas = serializer.Deserialize<List<compras_albaranes>>(lineasJsonModificadas);
            }
            catch (Exception ex)
            {
            ...
            using (MySqlConnection conexion = new MySqlConnection(connStr))
            {
                ...
                {
                    cmdCheck.CommandText = "SELECT COUNT(*) FROM compras_albaranes WHERE NUMALB = @num AND FACTURADO IN ('P','S')";
                    cmdCheck.Parameters.AddWithValue("@num", numAlb);
                    if (Convert.ToInt32(cmdCheck.ExecuteScalar()) > 0)
                    {
                        TempData["Error"] = "No se puede modificar un albarán de compra que ya ha sido facturado total o parcialmente.";
                        TempData["TipoError"] = "Editar";
                        return RedirectToAction("Index");
                    }
                }
                ...
                using (MySqlTransaction transaccion = conexion.BeginTransaction())
                {
                    try
                    {
                        ...
                        using (var cmdGetEstado = conexion.CreateCommand())
                        {
                            ...
                            cmdGetEstado.CommandText = "SELECT MAX(ESTADO) FROM compras_albaranes WHERE NUMALB = @num";
                        ...
                        using (var cmdDelete = conexion.CreateCommand())
                        {
                            ...
                            cmdDelete.CommandText = "DELETE FROM compras_albaranes WHERE NUMALB = @numalb";
                        ...
                        foreach (var linea in nuevasLineas)
                        {
                            using (var cmdInsert = conexion.CreateCommand())
                            {
                                ...
                                cmdInsert.CommandText = @"INSERT INTO compras_albaranes (
                                    NUMALB, FECHALB, NUMPEDIDO, NUMLINEAPED, CUENTA, FCOBRO, NUMLINEA, ARTI, DESARTI, 
                    ...
                    {
                        transaccion.Rollback();
                        TempData["Error"] = "Fallo al actualizar el albarán de compra: " + ex.Message;
                        TempData["TipoError"] = "Editar";
                    }
                }
            }
        ...
        public ActionResult Eliminar(string id)
        {
            ...
            using (MySqlConnection conexion = new MySqlConnection(connStr))
            {
                try
                {
                    ...
                    using (var cmd = conexion.CreateCommand())
                    {
                        cmd.CommandText = "DELETE FROM compras_albaranes WHERE NUMALB = @num";
                ...
                {
                    TempData["Error"] = "Error al eliminar albarán de compra: " + ex.Message;
                }
            }
            return RedirectToAction("Index");
        }
                ...
                using (MySqlTransaction transaccion = conexion.BeginTransaction())
                {
                    try
                    {
                        ...
                        using (var cmdCab = conexion.CreateCommand())
                        {
                            ...
                            cmdCab.CommandText = "SELECT DISTINCT CUENTA, FCOBRO, OBSERVACIONES FROM compras_pedidos WHERE NUMPEDIDO = @num LIMIT 1";
                        ...
                        foreach (var linea in lineasSeleccionadas)
                        {
                            ...
                            using (var cmdLin = conexion.CreateCommand())
                            {
                                ...
                                cmdLin.CommandText = @"
                                    SELECT CANTI, ARTI, DESARTI, UNIDAD, EUROS, DTOARTI, IVARTI, ALBARANADO,
                                           IFNULL((SELECT SUM(a.CANTI) FROM compras_albaranes a WHERE a.NUMPEDIDO = p.NUMPEDIDO AND a.NUMLINEAPED = p.NUMLINEA), 0) as YaAlbaranada
                                    FROM compras_pedidos p
                            ...
                            using (var cmdIns = conexion.CreateCommand())
                            {
                                ...
                                cmdIns.CommandText = @"INSERT INTO compras_albaranes (
                                    NUMALB, FECHALB, NUMPEDIDO, NUMLINEAPED, CUENTA, FCOBRO, NUMLINEA, ARTI, DESARTI, 
                                    UNIDAD, CANTI, EUROS, IVARTI, DTOARTI, ESTADO, ESTADOLIN, OBSERVACIONES
                                ) VALUES (
                            ...
                            using (var cmdUpd = conexion.CreateCommand())
                            {
                                ...
                                cmdUpd.CommandText = "UPDATE compras_pedidos SET ALBARANADO = @estAlb WHERE NUMPEDIDO = @num AND NUMLINEA = @linea";
                        ...
                        using (var cmdEst = conexion.CreateCommand())
                        {
                            ...
                            cmdEst.CommandText = @"
                                SELECT 
                                    ...
                                    COUNT(*) as Total
                                FROM compras_pedidos WHERE NUMPEDIDO = @num";
                        ...
                        using (var cmdGlobal = conexion.CreateCommand())
                        {
                            ...
                            cmdGlobal.CommandText = "UPDATE compras_pedidos SET ESTADO = @est WHERE NUMPEDIDO = @num";
        ...
        public JsonResult ExisteNumeroAlbaran(string numAlb)
        {
            ...
            using (MySqlConnection conexion = new MySqlConnection(connStr))
            {
                try
                {
                    ...
                    using (var cmd = conexion.CreateCommand())
                    {
                        cmd.CommandText = "SELECT COUNT(*) FROM compras_albaranes WHERE NUMALB = @num";
